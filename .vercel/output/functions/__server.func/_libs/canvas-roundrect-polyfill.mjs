...
//#region node_modules/canvas-roundrect-polyfill/roundRect.js
var roundRect_exports = /* @__PURE__ */ __exportAll({});
(() => {
	"use strict";
	Path2D.prototype.roundRect ??= roundRect;
	if (globalThis.CanvasRenderingContext2D) globalThis.CanvasRenderingContext2D.prototype.roundRect ??= roundRect;
	if (globalThis.OffscreenCanvasRenderingContext2D) globalThis.OffscreenCanvasRenderingContext2D.prototype.roundRect ??= roundRect;
	function roundRect(x, y, w, h, radii) {
		...
		} else throw new RangeError(`${getErrorMessageHeader(this)} ${radii.length} is not a valid size for radii sequence.`);
		const corners = [
			upperLeft,
			upperRight,
			lowerRight,
		...
		if (corners.some(({ x, y }) => !Number.isFinite(x) || !Number.isFinite(y))) return;
		if (negativeCorner) throw new RangeError(`${getErrorMessageHeader(this)} Radius value ${negativeCorner} is negative.`);
		...
		function parseRadiiArgument(value) {
			...
			if (type === "undefined" || value === null) return [0];
			if (type === "function") return [NaN];
			if (type === "object") {
				if (typeof value[Symbol.iterator] === "function") return [...value].map((elem) => {
					const elemType = typeof elem;
					if (elemType === "undefined" || elem === null) return 0;
					if (elemType === "function") return NaN;
					if (elemType === "object") return toDOMPointInit(elem);
		...
		function fixOverlappingCorners(corners) {
			...
			if (minFactor <= 1) for (const radii of corners) {
				radii.x *= minFactor;
				radii.y *= minFactor;
			}
		}
