...
//#region node_modules/image-blob-reduce/dist/image-blob-reduce.esm.mjs
var image_blob_reduce_esm_exports = /* @__PURE__ */ __exportAll({ default: () => imageBlobReduce });
	...
	ExifParser.prototype.filter = function(on_entry) {
		...
		Object.keys(ifds).forEach(function(ifd_no) {
			...
			ifds[ifd_no].entries.sort(function(a, b) {
				...
				} else if (entry.data_length <= 4) self.output.set(self.input.subarray(entry.data_offset - self.start, entry.data_offset - self.start + 4), entry_offset + 8);
				else {
		...
		Object.keys(ifds).forEach(function(ifd_no) {
			if (ifds[ifd_no].written_offset && ifds[ifd_no].link_offset) self.write_uint32(ifds[ifd_no].link_offset, ifds[ifd_no].written_offset);
		});
		...
		for (;;) {
			...
			if (byte1 === 255 && byte2 === 255) {
				...
				if (inside_scan) {
					...
					else inside_scan = false;
				}
				if (segment_code === 218) inside_scan = true;
			} else if (inside_scan) for (var pos = offset + 1;; pos++) {
	...
	module.exports.jpeg_exif_tags_filter = function(jpeg_bin, on_exif_entry) {
		...
		return module.exports.jpeg_segments_filter(jpeg_bin, function(segment) {
			...
			if (segment.code === 225 && segment.length >= 10 && jpeg_bin[segment.offset + 4] === 69 && jpeg_bin[segment.offset + 5] === 120 && jpeg_bin[segment.offset + 6] === 105 && jpeg_bin[segment.offset + 7] === 102 && jpeg_bin[segment.offset + 8] === 0 && jpeg_bin[segment.offset + 9] === 0) {
				...
				var header = /* @__PURE__ */ new Uint8Array(10);
...
function jpeg_rotate_canvas(env) {
	...
	var canvas;
	if (orientation & 4) canvas = this.pica.options.createCanvas(env.out_canvas.height, env.out_canvas.width);
	else canvas = this.pica.options.createCanvas(env.out_canvas.width, env.out_canvas.height);
...
function jpeg_attach_orig_segments(env) {
	...
	return Promise.all([this._getUint8Array(env.blob), this._getUint8Array(env.out_blob)]).then(function(res) {
		...
		env.out_blob = new Blob([data_out.slice(0, 2)].concat(segments).concat([data_out.slice(20)]), { type: "image/jpeg" });
		return env;
	});
}
...
ImageBlobReduce.prototype.toBlob = function(blob, options) {
	var env = {
		blob,
		opts: utils.assign({ max: Infinity }, options)
	};
	if (!this.initialized) {
		this.init();
...
ImageBlobReduce.prototype._getUint8Array = function(blob) {
	if (blob.arrayBuffer) return blob.arrayBuffer().then(function(buf) {
...
//#endregion
export { image_blob_reduce_esm_exports as t };
