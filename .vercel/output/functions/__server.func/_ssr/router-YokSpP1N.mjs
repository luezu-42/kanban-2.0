...
import { n as getPglite, r as getSql, t as ensureDbReady } from "./db-DiNR4WL6.mjs";
import { c as readAssetData, t as ASSET_ID } from "./board-rows.server-DSnb0Mmn.mjs";
import { t as commitWorkspacePayload } from "./workspace.server-fwycjmdx.mjs";
...
import { $t as getCurrentAdapter, A as decodeProtectedHeader, At as discriminatedUnion, Bt as unknown, C as toResponse, Dt as any, E as filterOutputFields, Et as _enum, Ft as object, It as optional, Jt as ATTR_HOOK_TYPE, Kt as withSpan, Lt as record, Mt as literal, Nt as looseObject, O as base64Url, Ot as array, Pt as number, Qt as getAuthTables, Rt as string, S as createRouter$1, Sn as defineErrorCodes, T as serializeSignedCookie, Xt as import_src, Yt as ATTR_OPERATION_ID, Zt as safeJSONParse, _ as defineRequestState, _n as isTest, a as betterFetch, an as initGetFieldName, b as getCurrentAuthContext, bn as kAPIErrorHeaderSymbol, c as isLoopbackHost, cn as createLogger, d as getIp, en as queueAfterTransactionHook, et as encode, f as deprecate, fn as env, ft as JWTExpired, g as isAPIError, gn as isProduction, h as createAuthMiddleware, hn as isDevelopment, i as refreshAccessToken, in as initGetModelName, j as jwtVerify, jt as email, k as decodeJwt, kt as boolean, l as createRateLimitKey, ln as logger, m as createAuthEndpoint, n as socialProviders, nn as runWithTransaction, o as createAuthorizationURL, on as generateId, p as normalizePathname, qt as ATTR_CONTEXT, r as validateAuthorizationCode, rn as getBetterAuthVersion, s as applyDefaultAccessTokenExpiry, sn as createRandomStringGenerator, t as SocialProviderListEnum, tn as runWithAdapter, u as findInvalidTrustedProxies, un as shouldPublishLog, v as hasRequestState, vn as APIError, w as serializeCookie, x as runWithEndpointContext, xn as BASE_ERROR_CODES, y as runWithRequestState, yn as BetterAuthError, zt as union } from "../_libs/@better-auth/core+[...].mjs";
import { c as TriangleAlert } from "../_libs/lucide-react.mjs";
import { t as Toaster } from "../_libs/sonner.mjs";
import { n as clsx, t as cva } from "../_libs/class-variance-authority+clsx.mjs";
import { t as twMerge } from "../_libs/tailwind-merge.mjs";
import { O as Slot } from "../_libs/@radix-ui/react-alert-dialog+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/router-YokSpP1N.js
var router_YokSpP1N_exports = /* @__PURE__ */ __exportAll({
	a: () => applyAppearance,
	c: () => SYNC_RETRY_EVENT,
	d: () => requestSyncRetry,
	f: () => triggerDownload,
	getRouter: () => getRouter,
	h: () => cn,
	i: () => subscribeSyncMessages,
	l: () => errorMessage,
	m: () => buttonVariants,
	n: () => clearWorkspaceSync,
	o: () => APPEARANCES,
	p: () => Button,
	r: () => enqueueWorkspaceSync,
	s: () => useProfileStore,
	t: () => router_exports,
	u: () => isOffline
});
...
var buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,background-color,box-shadow,transform,opacity] duration-150 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/70 focus-visible:ring-offset-2 focus-visible:ring-offset-bg disabled:pointer-events-none disabled:opacity-40 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 active:not-disabled:scale-[0.96]", {
	variants: {
		variant: {
			...
			secondary: "bg-surface text-fg shadow-border hover:bg-surface-hover",
...
function errorMessage(error, fallback) {
	if (isOffline()) return "You are offline. Changes stay on this device until you reconnect.";
	const raw = error instanceof Error ? error.message : typeof error === "string" ? error : "";
	if (/unauthorized|unlock|password|401/i.test(raw)) return "Session expired. Unlock the workspace again.";
	if (/failed to fetch|networkerror|load failed|network|timeout|aborterror/i.test(raw)) return "Could not reach the workspace. Check your connection and try again.";
	if (/quota|exceeded/i.test(raw)) return "This device is out of storage for a local copy.";
	return fallback;
}
var SYNC_RETRY_EVENT = "ledger-sync-retry";
function requestSyncRetry() {
...
var useProfileStore = create()(persist((set) => ({
	...
	markNoticeRead: (themeId, notice) => {
		set((state) => state.noticeRead[themeId] === notice ? state : { noticeRead: {
			...state.noticeRead,
			[themeId]: notice
		} });
	}
	...
	merge: (persisted, current) => {
		...
		return {
			...current,
			deviceId: typeof raw.deviceId === "string" && raw.deviceId ? raw.deviceId : current.deviceId,
			...
			noticeRead: raw.noticeRead && typeof raw.noticeRead === "object" ? Object.fromEntries(Object.entries(raw.noticeRead).filter((entry) => typeof entry[1] === "string")) : current.noticeRead
		};
	}
}));
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				...
				children: errorMessage(error, error.message || "An unexpected error occurred.")
			}),
...
var DB_NAME = "ledger-outbox";
var STORE = "items";
var KEY = "workspace";
var SYNC_TAG = "ledger-workspace";
var PERIODIC_TAG = "ledger-pull";
function openDb() {
...
async function enqueueWorkspaceSync(payload) {
...
async function clearWorkspaceSync() {
	const db = await openDb();
	await new Promise((resolve, reject) => {
		const tx = db.transaction(STORE, "readwrite");
...
async function requestBackgroundSync() {
	...
	try {
		const sync = (await navigator.serviceWorker.ready).sync;
		if (sync) {
...
async function registerPeriodicSync() {
	if (typeof navigator === "undefined" || !("serviceWorker" in navigator)) return;
	try {
		const periodic = (await navigator.serviceWorker.ready).periodicSync;
		if (!periodic) return;
		if ((await navigator.permissions.query({ name: "periodic-background-sync" })).state !== "granted") return;
		await periodic.register(PERIODIC_TAG, { minInterval: 9e5 });
	} catch {}
}
function subscribeSyncMessages(onFlush, onPull) {
...
async function registerLedgerWorker() {
	if (typeof navigator === "undefined" || !("serviceWorker" in navigator)) return;
	try {
...
var styles_default = "/assets/styles-EFoB6CsI.css";
var APP_NAME = "Ledger";
var THEME_BOOT = `(function(){try{window.EXCALIDRAW_ASSET_PATH="/excalidraw/";var r=localStorage.getItem("ledger-profile-v1");var t=r?JSON.parse(r).state.appearance:"dark";if(t!=="light"&&t!=="soft")t="dark";document.documentElement.setAttribute("data-theme",t);document.documentElement.style.colorScheme=t==="dark"?"dark":"light";}catch(e){window.EXCALIDRAW_ASSET_PATH="/excalidraw/";document.documentElement.setAttribute("data-theme","dark");}})();`;
var Route$6 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "A project planning workflow from backlog to done."
			},
			{
...
function ThemedToaster() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Toaster, {
		...
		toastOptions: { className: "!bg-bg-elevated !text-fg !border-border !shadow-lift !font-sans" }
	});
}
var $$splitComponentImporter$1 = () => import("./routes-COmF4DMp.mjs").then((n) => n.t);
var Route$5 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter$1, "component") });
var $$splitComponentImporter = () => import("./login-BNJmYgn_.mjs");
var Route$4 = createFileRoute("/login")({ component: lazyRouteComponent($$splitComponentImporter, "component") });
/**
* WebRTC signaling over Turso / local libSQL.
...
var PEER_TTL_SECONDS = 120;
var SIGNAL_TTL_SECONDS = 90;
...
async function roster(sql, room) {
	return (await sql.query(`SELECT peer_id, name FROM webrtc_peers
     WHERE room = $1 AND last_seen > datetime('now', '-' || $2 || ' seconds')
     ORDER BY peer_id LIMIT 32`, [room, String(PEER_TTL_SECONDS)])).map((r) => ({
		id: r.peer_id,
		name: r.name
	}));
}
...
async function prune(sql) {
	await Promise.all([sql.query(`DELETE FROM webrtc_signals WHERE created_at < datetime('now', '-' || $1 || ' seconds')`, [String(SIGNAL_TTL_SECONDS)]), sql.query(`DELETE FROM webrtc_peers WHERE last_seen < datetime('now', '-' || $1 || ' seconds')`, [String(PEER_TTL_SECONDS)])]);
}
...
async function handlePost({ request }) {
	...
	const value = body ?? {};
	if (typeof value.token !== "string" || !value.token) return json({
		ok: false,
		error: "unauthorized"
	}, 401);
	...
	try {
		const result = await commitWorkspacePayload(parsed.themes, parsed.activeThemeId, value.token, version);
		if (!result.ok && result.reason === "conflict") return json(result, 409);
...
var Route$2 = createFileRoute("/api/workspace-sync")({ server: { handlers: { POST: handlePost } } });
...
var Route$1 = createFileRoute("/api/assets/$id")({ server: { handlers: { GET: handleGet } } });
/**
		...
		hooks: { after: [{
			...
			handler: createAuthMiddleware(async (ctx) => {
				...
				if (returned instanceof Headers) {
					...
					const { setCookie } = await import("./ssr.mjs").then((n) => n.i).then((n) => n.t);
...
var rootRouteChildren = {
	...
	ApiWorkspaceSyncRoute: Route$2.update({
		id: "/api/workspace-sync",
		path: "/api/workspace-sync",
		getParentRoute: () => Route$6
	}),
	ApiAssetsIdRoute: Route$1.update({
		id: "/api/assets/$id",
		path: "/api/assets/$id",
		getParentRoute: () => Route$6
	}),
...
//#endregion
export { buttonVariants as a, enqueueWorkspaceSync as c, requestSyncRetry as d, router_YokSpP1N_exports as f, useProfileStore as h, applyAppearance as i, errorMessage as l, triggerDownload as m, Button as n, clearWorkspaceSync as o, subscribeSyncMessages as p, SYNC_RETRY_EVENT as r, cn as s, APPEARANCES as t, isOffline as u };
